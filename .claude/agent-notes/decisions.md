...
### Что и зачем
1. **Убрана интерактивная легенда EquityChart** (`app/components/EquityChart.tsx`). Раньше под графиком висел ряд чипов «● Имя $баланс» по каждому участнику + кнопка «сбросить», который дублировал боковую панель `ParticipantCard` (имена + балансы) и подписи на концах линий. На скрине Кирилла этот ряд воспринимался как «лишний верхний блок участников». Удалили вместе с локальной функцией `setHighlight` и пропом `onFocusChange` (всё выделение и так контролируется родителем через URL `?focus=`). Снять выделение — кнопка «Все» в заголовке графика и «Показать всех» в шапке боковой панели (уже были). Маленький индикатор «закрытые сделки» (две точки + надпись) ОСТАВИЛИ — это легенда трейд-маркеров, не дубль.
2. **Высота графика увеличена** с `h-[340px] sm:h-[420px]` до `h-[420px] sm:h-[520px] lg:h-[580px]`. Это естественно использует место, освободившееся от тулбара/легенды/футера, и делает equity-кривую доминирующим элементом экрана.
3. **Убран UI-индикатор «● live выкл / обновлено N сек назад»** из тулбара (`DashboardShell.tsx`). Кириллу не нужен видимый виджет статуса live-цен. Сигнал для разработки оставлен как `console.warn('[useLivePrices] stale: …')` в новом эффекте — поднимается только когда `live.stale && live.error`. Никаких эффектов на UX в норме.
4. **Мета-подпись под графиком теперь живая.** Было: `📊 Баланс и equity считаются автоматически из сделок · live-PnL из TwelveData по цене входа`. Стало: тот же текст из `competition.note` плюс суффикс ` · до следующего обновления: M:SS`, который тикает раз в секунду (нужен живой счётчик минут:секунд). Источник: `live.fetchedAt + NEXT_PUBLIC_PRICES_REFRESH_MS - now()`. Сбрасывается на полный интервал автоматически при каждом успешном fetch цен (через зависимость `live.fetchedAt`). Когда поллить нечего (нет открытых позиций с entryPrice) — суффикс пуст. Когда канал стейл — суффикс заменяется на ` · обновление вручную` (читаемая деградация без молчания).
5. **Убран ярлык «Реалити-торговля · live-дашборд»** из правой части шапки (`app/page.tsx`). Был дублем `competition.title` слева. Сам `competition.title` (из БД) остаётся.
6. **Убран футер** `competition.title · YYYY` (`app/page.tsx`). Тот же дубль заголовка, теперь снизу. БД не трогаем — это чисто UI-чистка. Контейнер `<footer>` удалён целиком.

### Что сознательно НЕ делал
- **Не трогал `competition.note` в БД и `data/competition.ts`.** Текст `Баланс и equity считаются автоматически из сделок · live-PnL из TwelveData по цене входа` приходит из Supabase (или из статического fallback). Кирилл попросил подпись `📊 Баланс и equity считаются автоматически из сделок · до следующего обновления: 1:42` — я склеил `note` (из источника правды) + динамический суффикс таймера. Если Кирилл хочет именно дословный текст без хвоста про live-PnL — поправить `competition_meta.note` в `/admin` (или в `data/competition.ts:86`). UI это подхватит без правок кода.
- **Не убирал «● ● ЗАКРЫТЫЕ СДЕЛКИ»** — это легенда маркеров закрытых сделок на графике (две цветные точки = profit/loss), а не дубль. Если потом захочется убрать — это правый блок в `EquityChart.tsx` после `{tradeMarkers.length > 0 && ...}`.
- **Не менял Realtime / focus / fetch-логику.** Контракт `useLiveCompetition`, `useLivePrices`, `lib/db.ts` — без изменений.
- **Не менял `EventTicker`** (бегущая лента закрытых сделок под лидербордом). Там тоже чипы участников, но они показывают **события** («Андрей закрыл LONG XAUUSD +$120»), а не балансы. На скрине это другой блок и Кирилл его не упоминал.

### Файлы
- `app/components/EquityChart.tsx` — удалена легенда имён + проп `onFocusChange` + локальная `setHighlight`; высота canvas увеличена.
- `app/components/DashboardShell.tsx` — удалён индикатор live в тулбаре; тулбар теперь = «📊 note + таймер» слева + Countdown справа; добавлены вычисления `refreshMs / showCountdown / nextUpdateSuffix / fmtCountdown`; добавлен `useEffect` для `console.warn` при стейле; удалён проп `onFocusChange={setFocusedName}` из `<EquityChart>`.
- `app/page.tsx` — удалён правый блок шапки «Реалити-торговля · live-дашборд» и `<footer>`.

### Валидация
- `npm run lint` — чисто (0 ошибок, 0 предупреждений).
- `npm run build` — чисто (Turbopack 2.7s, TS OK). Маршруты не изменились: `/` ƒ, `/admin` ƒ, `/api/prices` ƒ, `/api/tickers` ○ 5m. Ожидаемое сообщение `[db] ... falling back to static` — БД-схема старее текущего кода, graceful path (как в v10).
- `npm run test:pnl` — 19/19 pass.

### Caveats / открытые вопросы
- **Дословная мета-подпись.** Кирилл попросил ровно «📊 Баланс и equity считаются автоматически из сделок · до следующего обновления: 1:42». В коде сейчас выводится `📊 ${competition.note} · до следующего обновления: M:SS`. В БД `competition_meta.note = 'Баланс и equity считаются автоматически из сделок · live-PnL из TwelveData по цене входа'` → итог НЕ совпадает дословно: останется хвост про «live-PnL из TwelveData по цене входа». Если хочется ровно как в задаче — обновить `competition_meta.note` через `/admin` на «Баланс и equity считаются автоматически из сделок».
- **Клик по линии графика.** Раньше клик по линии вызывал `onParticipantClick` (открыть модалку). Так и осталось — этот контракт не тронут. Но клик-toggle для focus’а через легенду (которой больше нет) теперь недоступен; для смены focused-участника — либо клик по карточке справа (открывает модалку, после закрытия выставит focus = этот) либо ?focus=имя в URL. По спеке v10 это и было задумано.
- **Без Supabase env (статика).** `live.fetchedAt` всё равно появится после первого хода `useLivePrices` (он работает независимо от Supabase) → таймер будет видим. Если открытых позиций с `entryPrice` нет → `liveSymbols` пуст → таймер не показывается, всё корректно.
